...
modversion <- paste(unlist(SpaDES.core::moduleMetadata(module = 'Biomass_validationKNN', path = '..')$version), collapse = ".")
download.file(url = paste0("https://img.shields.io/badge/Biomass_validationKNN-", paste("v.", modversion),
                           "-%3CCOLOR%3E.png"),
              destfile = "figures/moduleVersionBadge.png",
              mode = 'wb')
```

[![module-version-Badge](figures/moduleVersionBadge.png)](https://github.com/CeresBarros/Biomass_validationKNN/commit/9250d68f905b8828d7965c733ab436e8a01fdf17)

[![Issues-badge](figures/issuesBadge.png)](https://github.com/PredictiveEcology/Biomass_validationKNN/issues)
...
calibration modules may both affect resulting biomass in different ways). See
...
the drop-down menu to see potential linkages. By default it does not validate
disturbed pixels, but the user is able to easily bypass this option. The
...
landscape-level. All calculations are done per replicate.
...
set of starting conditions and simulation mechanisms (i.e., the combination of
inputs to *Biomass_core* but also other modules that may be associated in
affecting vegetation dynamics), *Biomass_validationKNN* estimates sum of
negative log-likelihoods (SNLL) of simulated species biomasses, $\Delta$B (both
at the landscape and pixel-level), and species presences (at the
landscape-level), with respect to their observed counterparts.
...
**The computation of SNLL for** $\Delta$B is still under development, and the
...
-   `rawBiomassMapStart` -- raw biomass data used to initialize and parametrize
...
-   `rawBiomassMapEnd` -- raw biomass data used to validate the model after
    several simualtion years. By default, the module uses the stand biomass map
    from KNN for the year 2011, which is compared with the 10th year of a
    simulation initialised using the KNN 2001 data. The user must make sure this
...
to retrieve the observed kNN datasets used for validation[^biomass_validationknn-1].
...
`pixelGroupMapStk` input objects. See [list of input objects](#bvalid-inputs-list)
for more detail.
...
*Biomass_validationKNN* as a template. We expect the number of validation modules 
to increase as other validation approaches are developed based on project needs.

The general flow of *Biomass_validationKNN* processes is (note that
this module only runs once, i.e. in one "time step"):
...
2.  Calculation of summary variables for validation (`calculateValidVars` event), namely :
...
3.  Calculation of validation statistics (`validationStats` event), namely mean absolute deviations (MAD)
...
4.  Assessment of the relationship between observed $\Delta$B and observed
    ...
    of the observed data that produces scatterplots of $\Delta$B \~ $\Delta$Age of three types:
...
5.  Plots (`landscapeWidePlots`, `pixelLevelPlots` and `deltaBComparisons` events):
...
```{r fig-Biomass-validationKNNOutPlots, echo = FALSE, eval = TRUE, fig.show='hold', out.width = "70%", fig.cap = "(ref:Biomass-validationKNN) automatically generates plots showing a visual comparison between simulated and observed species presences (right) across the landscape, and relative species biomass per pixel (left)."}
...
knitr::include_graphics(normalizePath(c("figures/LandscapeComparisons_PresAbs.png",
                                        "figures/PixelComparisons_relB.png"), winslash = "/"))
```

```{r fig-Biomass-validationKNNOutPlots2, echo = FALSE, eval = TRUE, fig.show='hold', out.width = "70%", fig.cap = "A plot of landscape-wide mean absolute deviations (MAD) from (top to bottom) observed mean relative abundance, no. of presences, no. of pixels where the species is dominant and $\\Delta$B."}
knitr::include_graphics(normalizePath(c("figures/landscapeMAD.png"), winslash = "/"))
```

```{r fig-Biomass-validationKNNOutPlots3, echo = FALSE, eval = TRUE, fig.show='hold', out.width = "70%", fig.cap = "Diagnostic plot of observed changes in biomass and age $\\Delta$B and $\\Delta$Age, respectively)."}
knitr::include_graphics(normalizePath(c("figures/observedDeltaBDeltaAge.png"), winslash = "/"))
```

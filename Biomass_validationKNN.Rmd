...
simulationOutputs <- lapply(mySimExperiment, FUN = function(x, localSimPaths) {
  ...
  DT
}, localSimPaths = getPaths())
simulationOutputs <- rbindlist(simulationOutputs)

validationPaths <- getPaths()
...
validationParams <- list(
  Biomass_validationKNN = list(
    "sppEquivCol" = params(mySimInit)$sppEquivCol
...
noFires <- st_as_sf(mySimExperiment$sim1_rep1$studyArea)
noFires <- noFires[0,] 
...
and saves the validation figures in `getPaths$outputPath`
